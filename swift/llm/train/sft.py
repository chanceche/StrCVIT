...
from ..dataset import (AddLengthPreprocessor, EncodePreprocessor, AddTextEncodePreprocessor, IterablePackingDataset, LazyLLMDataset,
...
def set_peft_model_state_dict(model, peft_model_state_dict, adapter_name="default"):
    ...
    if config.peft_type in (
        PeftType.LORA, PeftType.ADALORA, PeftType.IA3, PeftType.DORA,
        PeftType.STRLORA, PeftType.SMOLORA, PeftType.MOELORA
    ):
...
def init_router_distill_ema_from_loaded_adapter(model):
    loaded = 0
    for module in model.modules():
        ...
        module_loaded = False
        if getattr(module, 'attn_proj_dim', 0) > 0 and hasattr(module, 'lora_attn_wq'):
            if not getattr(module, 'freeze_qk', False):
                if hasattr(module, 'lora_ema_attn_wq_weight') and adapter_name in module.lora_attn_wq:
                    module._set_ema_parameter(
                        module.lora_ema_attn_wq_weight,
                        adapter_name,
                        module.lora_attn_wq[adapter_name].weight,
                        reference_tensor=module.lora_attn_wq[adapter_name].weight,
                    )
                    module_loaded = True
                if hasattr(module, 'lora_ema_attn_wk_weight') and adapter_name in module.lora_attn_wk:
                    module._set_ema_parameter(
                        module.lora_ema_attn_wk_weight,
                        adapter_name,
                        module.lora_attn_wk[adapter_name].weight,
                        reference_tensor=module.lora_attn_wk[adapter_name].weight,
                    )
                    module_loaded = True
            if (
                not getattr(module, 'freeze_expo', False)
                and hasattr(module, 'lora_ema_attn_expert_proto')
                and adapter_name in module.lora_attn_expert_proto
            ):
                module._set_ema_parameter(
                    module.lora_ema_attn_expert_proto,
                    adapter_name,
                    module.lora_attn_expert_proto[adapter_name],
                    reference_tensor=module.lora_attn_expert_proto[adapter_name],
                )
                module_loaded = True
        elif (
            getattr(module, 'subtopk', 0) > 0
            and getattr(module, 'attn_proj_dim', 0) <= 0
            and hasattr(module, 'lora_sub_router')
            and hasattr(module, 'lora_ema_sub_router_weight')
            and adapter_name in module.lora_sub_router
        ):
            module._set_ema_parameter(
                module.lora_ema_sub_router_weight,
                adapter_name,
                module.lora_sub_router[adapter_name].weight,
...
def load_model_from_previous_task(model, previous_task_model_path, init_missing_router_ema=False):
    def load_from_local_or_hf(path_or_repo, filename):
        local_path = os.path.join(path_or_repo, filename)
        if os.path.exists(local_path):
            return torch.load(local_path, map_location='cpu')
        from huggingface_hub import hf_hub_download
        cache_file = hf_hub_download(repo_id=path_or_repo, filename=filename)
    ...
    adapters_weights = load_from_local_or_hf(previous_task_model_path, 'adapter_model.bin')
    # new_adapters_weights={}
    # for k,v in adapters_weights.items():
    #     if "lora_A" in k:
    #         new_adapters_weights[k] = v    
    # load_result = set_peft_model_state_dict(model, new_adapters_weights, adapter_name="default")
    load_result = set_peft_model_state_dict(model, adapters_weights, adapter_name="default")
    if hasattr(model, 'load_router_distill_ema_state'):
        try:
            ema_state = load_from_local_or_hf(previous_task_model_path, ROUTER_DISTILL_EMA_NAME)
            loaded = model.load_router_distill_ema_state(ema_state)
            print(f'Loaded router distill EMA state for {loaded} modules.')
        except Exception:
            if init_missing_router_ema:
                loaded = init_router_distill_ema_from_loaded_adapter(model)
                print(f'Initialized router distill EMA for {loaded} modules from previous adapter weights.')
...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def _prepare_dataset(self):
        ...
        if args.dataset:
            ...
            if "sample" in args.train_type:
            #     train_dataset, val_dataset = self._encode_dataset_add_text(train_dataset, val_dataset, pre_process=pre_process)
            # else:
    ...
    def run(self):
        ...
        self.args.training_args.enable_ewc = self.args.ewc_lambda > 0 and self.args.previous_task_model_path is not None
        if args.previous_task_model_path is not None:
            # load model from previous task
            init_missing_router_ema = args.train_type == 'strlora' and args.sample_router_distill_lambda > 0
            load_model_from_previous_task(self.model, args.previous_task_model_path, init_missing_router_ema)
            
        if args.train_aligner and not args.lora_aligner:
            for n,p in self.model.named_parameters():
                if 'mlp1.1' in n or "mlp1.3" in n:
    ...
    def train(self, trainer):
        ...
        try:
            ...
            trainer.save_state()
            trainer.save_trained_model(self.args)

        finally:
            trainer.accelerator.wait_for_everyone()
            res = self._save_trainer_state(trainer)
            # Save router stats if available
            if os.environ.get('DISABLE_ROUTER_STATS') != '1' and hasattr(self.model, "save_router_stats"):
                stats_path = os.path.join(trainer.args.output_dir, "router_stats.json")
                self.model.save_router_stats(stats_path)
            trainer.accelerator.wait_for_everyone()
            # if self.args.use_flash_ckpt:
    ...
    def _prepare_callbacks(self):
        from .callback import DynamicLayerActivationCallback, SampleRouterDistillCallback, TrainerAdapterCallback
        args = self.args
        callbacks = []
        ...
        if args.train_type == 'strlora' and args.sample_router_distill_lambda > 0:
            callbacks.append(SampleRouterDistillCallback())
