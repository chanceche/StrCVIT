...
def prepare_adapter(args: TrainArguments, model, *, template=None, train_dataset=None, task_type=None):
    ...
    if args.lora_aligner:
        target_modules = ['up_proj', 'o_proj', 'down_proj', 'gate_proj', 'v_proj', 'q_proj', 'k_proj','mlp1.1','mlp1.3']
    else:
    ...
    if args.train_type == 'strlora':
        from StrLoRA.peft import TaskType, get_peft_model
        from StrLoRA.peft.tuners.strlora import StrLoRAConfig
        kwargs = { 
                "task_embedding_dim":args.task_embedding_dim,
                "expert_num": args.expert_num,
                "subtopk": args.subtopk,
                "attn_proj_dim": args.attn_proj_dim,
                "sample_router_distill_lambda": args.sample_router_distill_lambda,
                "sample_router_distill_beta": args.sample_router_distill_beta,
                "sample_router_distill_tau": args.sample_router_distill_tau,
                "freeze_qk": args.freeze_qk,
                "freeze_expo": args.freeze_expo,
            }

        lora_config = StrLoRAConfig(
                r= args.lora_rank,
                lora_alpha= args.lora_alpha,
                target_modules=target_modules,
                lora_dropout= args.lora_dropout,
                bias= args.lora_bias,
                topk = args.topk,
                task_type=TaskType.CAUSAL_LM_STRLORA,
                **kwargs
            )
        model = get_peft_model(model, lora_config)
        if args.freeze_attn:
            freeze_modules = ["lora_attn_wq","lora_attn_wk","lora_attn_expert_proto","lora_router"]
            for name, param in model.named_parameters():
                if  name in freeze_modules or any([fm in name for fm in freeze_modules]):
                    param.requires_grad = False
        elif args.sample_router_distill_lambda > 0 and args.freeze_qk:
            freeze_modules = ["lora_attn_wq","lora_attn_wk","lora_router"]
            for name, param in model.named_parameters():
                if  name in freeze_modules or any([fm in name for fm in freeze_modules]):
                    param.requires_grad = False
        if args.freeze_expo:
            freeze_modules = ["lora_router","lora_attn_expert_proto"]
            for name, param in model.named_parameters():
                if name in freeze_modules or any([fm in name for fm in freeze_modules]):
                    param.requires_grad = False
        if args.freeze_textrouter:
            freeze_modules = ["lora_router"]
            for name, param in model.named_parameters():
                if name in freeze_modules or any([fm in name for fm in freeze_modules]):
                    param.requires_grad = False

        
    elif args.train_type == 'smolora':
        from StrLoRA.peft import TaskType, get_peft_model
        from StrLoRA.peft.tuners.smolora import SMoLoraConfig

        if not args.ins_emb:
            raise ValueError('`--ins_emb` is required when `--train_type smolora`.')
        if not os.path.exists(args.ins_emb):
            raise FileNotFoundError(f'`ins_emb` not found: {args.ins_emb}')

        with open(args.ins_emb, 'rb') as f:
            ins_emb = pickle.load(f)
        if hasattr(ins_emb, 'tolist'):
            ins_emb = ins_emb.tolist()

        ins_emb_rows = len(ins_emb) if isinstance(ins_emb, (list, tuple)) else None
        ins_emb_dim = None
        if ins_emb_rows:
            first_row = ins_emb[0]
            if isinstance(first_row, (list, tuple)):
                ins_emb_dim = len(first_row)

        kwargs = {
            'expert_num': args.expert_num if args.expert_num is not None else 4,
            'ins_type': args.ins_type if args.ins_type is not None else 0,
            'ins_emb': ins_emb,
        }
        ...
        logger.info(
            ...
            f'bias={lora_config.bias}, '
            f'expert_num={lora_config.expert_num}, '
            f'ins_type={lora_config.ins_type}, '
            f'ins_emb_shape=({ins_emb_rows}, {ins_emb_dim}), '
            f'ins_emb_path={args.ins_emb}, '
            f'target_modules={lora_config.target_modules})'
        )

    elif args.train_type == 'moelora':
        from StrLoRA.peft import TaskType, get_peft_model
        from StrLoRA.peft.tuners.moelora import MoELoraConfig

        kwargs = {
            'task_embedding_dim': args.task_embedding_dim,
        }
        if args.expert_num is not None:
            kwargs['expert_num'] = args.expert_num
        ...
        lora_config = MoELoraConfig(
            ...
            bias=args.lora_bias,
            topk=args.topk,
            task_type=TaskType.CAUSAL_LM_MOELORA,
            **kwargs,
        )
        model = get_peft_model(model, lora_config)
        ...
        from StrLoRA.peft import LoraConfig, get_peft_model
