...
def eval_cider(annotation_file, result_file):
    ...
    for result in results:
        ...
        if question_id in annotations_dict:
            ...
            res[question_id] = [result['text']]  # Use question_id as key; prediction as list

            # Put all references into gts
            # Ensure gts values are lists even for a single reference
            answer = annotations_dict[question_id]['answer']
            gts[question_id] = [answer] if isinstance(answer, str) else answer  # Ensure reference is a list
