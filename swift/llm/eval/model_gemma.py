...
from transformers import AutoModel, AutoTokenizer, AutoConfig, AutoProcessor, Gemma3ForConditionalGeneration
...
def eval_model(args):
    ...
    if args.model_path:
        ...
        model = Gemma3ForConditionalGeneration.from_pretrained(
            model_base, device_map="auto", torch_dtype=torch.bfloat16,
        )
        processor = AutoProcessor.from_pretrained("<MODEL_ROOT>/gemma-3-4b-it")
        ...
        if "SMOLORA" in model_path_upper:
        ...
        print()
        model = Gemma3ForConditionalGeneration.from_pretrained(
            "gemma-3-4b-it", device_map="auto", torch_dtype=torch.bfloat16,
        )
        processor = AutoProcessor.from_pretrained("<MODEL_ROOT>/gemma-3-4b-it")
    ...
    for i, line in enumerate(tqdm(questions, disable=not show_progress, desc="Evaluating", ncols=100)):
        ...
        if "image" in line.keys():
            ...
            content.append({"type": "image", "url": f"{image_path}"})
        ...
        try:
            ...
            if hasattr(model, '_resolve_routing_text_embeds'):
                routing_text = question if question else ' '
                inputs['text_ids'] = processor.tokenizer(
                    routing_text,
                    return_tensors='pt',
