...
class Template(ProcessorMixin):
    ...
    def encode(self,
        ...
        for encoded in batched:
            ...
            if 'smolora_ins_idx' in chosen.extra_kwargs:
                encoded['smolora_ins_idx'] = chosen.extra_kwargs['smolora_ins_idx']
    ...
    def pre_forward_hook(self, model: nn.Module, args, kwargs):
        ...
        if 'text_embeds' in kwargs:
            kwargs.pop('text_ids', None)
        base_model = self.get_base_model(model)
    ...
    def _data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        ...
        if self.padding_free:
            ...
            for key in ['labels', 'loss_scale', 'position_ids', 'token_type_ids', 'text_ids', 'text_embeds']:
                val = [b[key] for b in batch if b.get(key) is not None]
                if val:
                    res[key] = val
            val = [b['smolora_ins_idx'] for b in batch if b.get('smolora_ins_idx') is not None]
            if val:
                res['smolora_ins_idx'] = torch.tensor(val, dtype=torch.long)
        ...
        pad_values = [self.tokenizer.pad_token_id, 0., 0, -100, 0., 0., 0, 0,self.tokenizer.pad_token_id, 0.]
        ...
        for key in keys_to_check:
            ...
            if val is not None:
                ...
                if isinstance(val, torch.Tensor):
                    flat_val = val.detach().cpu().reshape(-1).tolist()
                    if len(flat_val) > 20:
                        val_summary = f'shape={tuple(val.shape)}, len={len(flat_val)}, head={flat_val[:10]}, tail={flat_val[-10:]}'
                    else:
                        val_summary = f'shape={tuple(val.shape)}, value={flat_val}'
                elif isinstance(val, (list, tuple)) and (not val or not isinstance(val[0], (list, tuple))):
                    flat_val = list(val)
                    if len(flat_val) > 20:
                        val_summary = f'len={len(flat_val)}, head={flat_val[:10]}, tail={flat_val[-10:]}'
                    else:
                        val_summary = flat_val
                else:
                    val_summary = f'type={type(val).__name__}, len={len(val) if hasattr(val, "__len__") else "unknown"}'
                logger.info(f'[{key_upper}_IDS] {val_summary}')
                ...
                if isinstance(val, (list, tuple, torch.Tensor)):
                    ...
                    if isinstance(val_str, str) and len(val_str) > 300:
                        val_str = f'{val_str[:300]} ... [truncated, chars={len(val_str)}]'
