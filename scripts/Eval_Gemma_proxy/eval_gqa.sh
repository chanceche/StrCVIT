...
if [ ! -n "$3" ] ;then
    Instruction="<STRCVIT_DATASET_DIR>/test/GQA/test_proxy.json"
else
    Instruction=<STRCVIT_DATASET_DIR>/$3/GQA/test_proxy.json
fi
...
python -m swift.llm.eval.convert_gqa_for_eval --src $output_file --dst $RESULT_DIR/$STAGE/testdev_balanced_predictions.json

python -m swift.llm.eval.eval_gqa --tier testdev_balanced --path $RESULT_DIR/$STAGE --output-dir $RESULT_DIR/$STAGE
