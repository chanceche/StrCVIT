...
if [ ! -n "$2" ] ;then
    model_base=<MODEL_ROOT>/InternVL3_5-1B-Pretrained
else
...
if [ ! -n "$3" ] ;then
    Instruction="<STRCVIT_DATASET_DIR>/test/ImageNet/test_proxy.json"
else
    Instruction=<STRCVIT_DATASET_DIR>/$3/ImageNet/test_proxy.json
fi
