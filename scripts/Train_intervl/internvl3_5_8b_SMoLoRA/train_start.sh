...
if [ ! -f "$INS_EMB_PATH" ]; then
    ...
    echo "   Generate StrLoRA sample-level files first or set INS_EMB_PATH explicitly."
    exit 1
fi
...
MAX_PIXELS=802816 \
swift sft \
    --model <MODEL_ROOT>/InternVL3_5-8B-Pretrained \
    --dataset "$DATA_PATH" \
    ...
    --report_to none \
    --max_length 4096 \
    --output_dir <STRCVIT_METHODS_DIR>/checkpoints/StrCVIT/data_001/SMoLoRA \
    --warmup_ratio 0.05 \
