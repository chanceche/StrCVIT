...
################## CONFIG ##################
MODEL_PATH="<MODEL_ROOT>/InternVL3_5-8B-Pretrained"
...
INS_EMB_ROOT="${INS_EMB_ROOT:-${SCRIPT_DIR}/embeddings}"
################## CONFIG ##################
...
prepare_smolora_dataset() {
    ...
    if [ ! -f "$src" ]; then
        echo "Missing source dataset file: $src"
        exit 1
    fi
...
if [ -n "$1" ]; then
    ...
    output_dir="${CKPT_ROOT}/${task_id}/SMoLoRA"
    pre_dir="${CKPT_ROOT}/data_${prev_num}/SMoLoRA"
...
if [ ! -f "$ins_emb_path" ]; then
    echo "Missing instruction embedding file: $ins_emb_path"
    echo "   Generate StrLoRA sample-level files first or set INS_EMB_ROOT explicitly."
    exit 1
fi

if [ ! -f "$data_path" ]; then
    echo "Failed to prepare SMoLoRA dataset file: $data_path"
    exit 1
fi
