    ...
    def save_router_stats(self, path=None):
        ...
        if stats:
            try:
                ...
                print(f"[MoELora] Router statistics saved to {path}")
            except Exception as e:
                print(f"[MoELora] Failed to save router statistics: {e}")
        else:
            print(f"[MoELora] No router statistics collected. File {path} was not created.")
...
class MoELoraLayer(LoraLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            self.lora_A.update(nn.ModuleDict({adapter_name: MoELinearA(self.in_features, r, self.expert_num)}))
            self.lora_B.update(nn.ModuleDict({adapter_name: MoELinearB(r, self.out_features, self.expert_num)}))
...
class MoELoraLinear(nn.Linear, MoELoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        MoELoraLayer.__init__(self, in_features=in_features, 
                               out_features=out_features, 
                               expert_num=self.expert_num)
        
        # Initialize expert counts
        self.register_buffer('expert_counts', torch.zeros(self.expert_num))
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.disable_adapters:   # No adapter
            ...
            if self.topk > 0:
                router_probs = torch.softmax(router, dim=-1)
                topk_val, topk_idx = torch.topk(router_probs, k=self.topk, dim=-1)
                router = torch.zeros_like(router_probs).scatter_(-1, topk_idx, topk_val)
            else:
            ...
            if not self.training or self.topk > 0:
                with torch.no_grad():
                    flat_router = router.view(-1, self.expert_num)
                    if self.topk > 0:
                        # Count non-zero entries
                        usage = (flat_router > 0).float().sum(dim=0)
                    else:
                        # For softmax, sum probabilities to get distribution
                        # We sum (instead of mean) to keep the scale consistent with token counts (sum of probs = num_tokens)
                        # This allows calculating the correct global average distribution later.
                ...
                for i in range(self.expert_num):
                    result += ( # lora process
                        self.lora_B[self.active_adapter].loraB[i](
                            self.lora_A[self.active_adapter].loraA[i](x),
                        )
                        * self.scaling[self.active_adapter]
...
class MoERouter(nn.Module):
    ...
    def __init__(self, config: MoELoraConfig):
        super().__init__()
