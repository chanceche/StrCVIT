...
class PeftModelForCausalLMLORAMOE(PeftModelForCausalLM):
    ...
    def update_router_distill_ema(self):
        if hasattr(self.base_model, "update_router_distill_ema"):
            self.base_model.update_router_distill_ema()

    def get_router_distill_ema_state(self):
        if hasattr(self.base_model, "get_router_distill_ema_state"):
            return self.base_model.get_router_distill_ema_state()
        return {}

    def load_router_distill_ema_state(self, state):
        if hasattr(self.base_model, "load_router_distill_ema_state"):
            return self.base_model.load_router_distill_ema_state(state)
        return 0

    def _get_routing_embedding_layer(self):
    ...
    def _build_routing_text_embeds(self, token_ids):
        ...
        emb_layer = self._get_routing_embedding_layer()
        if emb_layer is None:
            return None

        device = emb_layer.weight.device
        return emb_layer(token_ids.to(device=device))

    def _resolve_routing_text_embeds(
        self,
        input_ids=None,
        inputs_embeds=None,
        text_embeds=None,
        ...
        if text_ids is not None:
            return self._build_routing_text_embeds(text_ids)
        if input_ids is not None:
            return self._build_routing_text_embeds(input_ids)
        if inputs_embeds is not None:
    ...
    def forward(self, 
        ...
        smolora_ins_idx = kwargs.pop("smolora_ins_idx", None)
        text_embeds = self._resolve_routing_text_embeds(
            input_ids=input_ids,
            inputs_embeds=inputs_embeds,
        ...
        self.base_model._routing_text_embeds = text_embeds.detach() if text_embeds is not None else None
        self.base_model._smolora_batch_ins_idx = smolora_ins_idx
        if hasattr(self.base_model, "clear_router_distill_loss"):
            self.base_model.clear_router_distill_loss()
        ...
        try:
            if not isinstance(peft_config, PromptLearningConfig):
                ...
                if hasattr(self.base_model, "get_router_distill_loss"):
                    distill_loss = self.base_model.get_router_distill_loss()
            ...
            self.base_model._smolora_batch_ins_idx = None
    ...
    def generate(self, **kwargs):
        ...
        routing_text_embeds = self._resolve_routing_text_embeds(
            input_ids=input_ids,
            inputs_embeds=inputs_embeds,
            text_embeds=text_embeds,
            text_ids=text_ids,
        ...
        self.base_model._routing_text_embeds = (
            routing_text_embeds.detach() if routing_text_embeds is not None else None
        )
        self.base_model.prepare_inputs_for_generation = self.prepare_inputs_for_generation
        if hasattr(self.base_model, "model"):
            self.base_model.model.generation_config = self.generation_config
    ...
    def _extract_routing_text(self, question: Union[str, List[str]]) -> str:
        ...
        if isinstance(question, str):
            q = re.sub(r"<image>\s*", "", question).strip()
            if q:
                texts.append(q)
        elif isinstance(question, (list, tuple)):
            for item in question:
                if isinstance(item, str):
                    q = re.sub(r"<image>\s*", "", item).strip()
                    if q:
                        texts.append(q)
    ...
    def _build_text_embeds(self, tokenizer, routing_text: str) -> torch.Tensor:
        ...
        if emb_layer is None:
            return None
        text_embeds = emb_layer(text_ids)  # [1, Lt, H]
        return text_embeds
    ...
    def _build_batch_text_embeds(self, tokenizer, routing_texts: List[str]) -> torch.Tensor:
        ...
        if emb_layer is None:
            return None
        text_embeds = emb_layer(text_ids)
        return text_embeds
    ...
    def chat(
        ...
        **kwargs
        ...
        routing_text = self._extract_routing_text(question)

        text_embeds = None
        routing_text = routing_text if routing_text else " "
        text_embeds = self._build_text_embeds(tokenizer, routing_text)

        self.base_model._routing_text_embeds = (
            text_embeds.detach() if text_embeds is not None else None
        )

        try:
            return self.base_model.chat(
                tokenizer=tokenizer,
    ...
    def batch_chat(
        ...
        **kwargs
        ...
        if isinstance(questions, (list, tuple)):
            routing_texts = self._extract_batch_routing_texts(list(questions))
            if len(routing_texts) > 0:
                text_embeds = self._build_batch_text_embeds(tokenizer, routing_texts)
        else:
            routing_text = self._extract_routing_text(questions)
            routing_text = routing_text if routing_text else " "
            text_embeds = self._build_text_embeds(tokenizer, routing_text)
        ...
        try:
            return self.base_model.batch_chat(
                tokenizer=tokenizer,
                pixel_values=pixel_values,
                questions=questions,
