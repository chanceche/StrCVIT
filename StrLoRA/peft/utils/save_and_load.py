...
def get_peft_model_state_dict(model, state_dict=None, adapter_name="default"):
    ...
    if config.peft_type in (
        ...
        bias = config.bias
        if bias == "none":
            to_return = {k: state_dict[k] for k in state_dict if "lora_" in k and "lora_ema_" not in k}
        elif bias == "all":
            to_return = {k: state_dict[k] for k in state_dict if ("lora_" in k and "lora_ema_" not in k) or "bias" in k}
        elif bias == "lora_only":
            to_return = {}
            for k in state_dict:
